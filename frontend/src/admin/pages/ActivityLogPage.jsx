...
export function ActivityLogPage() {
  ...
  const columns = [
    { key: 'createdAt', label: 'Date', render: item => new Date(item.createdAt).toLocaleString(locale) },
    { key: 'adminName', label: 'Administrator' },
    { key: 'action', label: 'Action', render: item => t(item.action) },
    { key: 'entityType', label: 'Content type', render: item => t(contentName(item.entityType)) },
    { key: 'summary', label: 'Details', render: item => <div className="activity-details"><strong>{t(readableDetails(item))}</strong></div> },
  ]
  ...
  return <>
    ...
    <section className="admin-panel">
      <div className="communication-summary activity-filter-bar">
        <input placeholder={t('Search activity…')} value={filters.search} onChange={event => update({ search: event.target.value })} />
        ...
        <label className="activity-day-filter"><span>{t('Day')}</span><input type="date" value={filters.day} onChange={event => update({ day: event.target.value })} /></label>
        <select aria-label="Action" value={filters.action} onChange={event => update({ action: event.target.value })}>
          <option value="">{t('All actions')}</option>
          {options.actions.map(action => <option key={action} value={action}>{t(action)}</option>)}
        </select>
        <select aria-label="Content type" value={filters.entityType} onChange={event => update({ entityType: event.target.value })}>
          ...
          {options.entityTypes.map(type => <option key={type} value={type}>{t(contentName(type))}</option>)}
        </select>
        <button className="activity-clear-filters" type="button" onClick={() => setFilters(initialFilters)}>{t('Clear filters')}</button>
      </div>
      <DataTable columns={columns} rows={data.items} emptyText="No matching activity." />
      <div className="admin-pagination"><span>{data.totalCount} {t('actions')}</span><div>
        <button disabled={filters.page <= 1} onClick={() => setFilters(current => ({ ...current, page: current.page - 1 }))}>← {t('Previous')}</button>
        <span>{t('Page')} {filters.page}</span>
        <button disabled={filters.page * filters.pageSize >= data.totalCount} onClick={() => setFilters(current => ({ ...current, page: current.page + 1 }))}>{t('Next')} →</button>
      </div></div>
    </section>
  </>
}
