...
export function ExistingContentPage({ type }) {
  ...
  const [filters, setFilters] = useState({ search: '', status: '', articleType: '', page: 1, pageSize: 25 })
  const [data, setData] = useState(null); const [error, setError] = useState('')
  const load = () => {
    setError('')
    return adminApi.existingContent(type, filters)
      .then(result => setData(type === 'news'
        ? { ...result, items: result.items.map(item => ({ ...item, titleEn: item.titleEn?.trim().toLowerCase() === 'untitled' ? '' : item.titleEn })) }
        : result))
      .catch(e => setError(e.response?.data?.detail ?? `The ${config.title.toLowerCase()} could not be loaded.`))
  }
  useEffect(() => { setData(null); const timer = setTimeout(() => { void load() }, filters.search ? 180 : 0); return () => clearTimeout(timer) }, [type, filters.search, filters.status, filters.articleType, filters.page])
  const totalPages = Math.max(1, Math.ceil((data?.totalCount ?? 0) / filters.pageSize))
  return <><PageHeader eyebrow={config.eyebrow} title={config.title} description={config.description} actions={type === 'news' ? <Link className="admin-primary-button" to="/admin/news/new">+ Add news article</Link> : null} />
    ...
    <section className="admin-panel"><div className="communication-summary"><input placeholder={`Search ${config.title.toLowerCase()}…`} value={filters.search} onChange={e => setFilters({ ...filters, search: e.target.value, page: 1 })} /><select value={filters.status} onChange={e => setFilters({ ...filters, status: e.target.value, page: 1 })}><option value="">All publication states</option><option value="published">Published</option><option value="draft">Draft</option></select>{type === 'news' && <select aria-label="Article ownership" value={filters.articleType} onChange={e => setFilters({ ...filters, articleType: e.target.value, page: 1 })}><option value="">All news types</option><option value="authored">Our articles</option><option value="external">External articles</option></select>}<span>{data?.totalCount ?? 0} records</span></div></section>
