...
export function UsersRolesPage() {
  ...
  const columns = [{ key: 'displayName', label: 'Administrator' }, { key: 'email', label: 'Email' }, { key: 'role', label: 'Role' }, { key: 'isActive', label: 'Status', render: x => <StatusBadge status={x.isActive ? 'Active' : 'Inactive'} /> }, { key: 'lastLoginAt', label: 'Last login', render: x => x.lastLoginAt ? new Date(x.lastLoginAt).toLocaleString() : 'Never' }, { key: 'actions', label: '', render: x => <div className="table-actions"><button onClick={() => edit(x)}>Edit</button>{x.isActive && <button onClick={() => reset(x)}>Reset password</button>}{x.isActive ? <button className="danger" onClick={() => remove(x)}>Remove access</button> : <button className="danger" onClick={() => destroy(x)}>Delete permanently</button>}</div> }]
  return <><PageHeader eyebrow="Administration" title="Users & Roles" description="Create administrators, assign roles, reset passwords and control access." /><section className="admin-panel"><DataTable columns={columns} rows={items} /></section><section className="admin-panel admin-form"><h2>{selected ? 'Edit administrator' : 'Create administrator'}</h2><form className="form-grid" onSubmit={save}><label>Full name<input required value={form.displayName} onChange={e => setForm({ ...form, displayName: e.target.value })} /></label><label>Email<input required type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></label><label>Role<select value={form.role} onChange={e => setForm({ ...form, role: e.target.value })}><option value="ContentEditor">Content Editor</option><option value="SuperAdmin">Super Admin</option></select></label><label className="admin-check"><input type="checkbox" checked={form.isActive} onChange={e => setForm({ ...form, isActive: e.target.checked })} /> Active account</label>{!selected && <label>Password<input required minLength="12" type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} /></label>}<div className="full table-actions"><button className="admin-primary-button" type="submit">{selected ? 'Save administrator' : 'Create administrator'}</button>{selected && <button type="button" onClick={() => { setSelected(null); setForm(blank) }}>Cancel</button>}</div></form></section><Toast message={toast} onClose={() => setToast('')} /></>
}
