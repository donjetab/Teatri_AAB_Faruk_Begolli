...
export function MediaPicker({ label, value, currentUrl, type = 'image/', onChange, onSelect }) {
  ...
  return <div className="media-picker-field"><span>{label}</span><div>
    {previewUrl ? previewIsVideo ? <video src={resolveMediaUrl(previewUrl)} muted /> : <img src={resolveMediaUrl(previewUrl)} alt="" /> : <i>◇</i>}
    <strong>{current?.fileName ?? (value ? `Media #${value}` : isMixed ? 'No media selected' : isVideo ? 'No local video selected' : 'No image selected')}</strong>
