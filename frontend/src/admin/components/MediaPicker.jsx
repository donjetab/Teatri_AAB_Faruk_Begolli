...
export function MediaPicker({ label, value, currentUrl, type = 'image/', onChange, onSelect, compact = false }) {
  ...
  return <div className={`media-picker-field ${compact ? 'compact' : ''}`}>{compact ? <button className="admin-outline-button" type="button" onClick={() => setOpen(true)}>{label}</button> : <><span>{label}</span><div>
