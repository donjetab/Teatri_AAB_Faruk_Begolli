...
export function CreditRepeater({ showId, onSaved }) {
  const [credits, setCredits] = useState(null)
  const [types, setTypes] = useState([])
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    adminApi.showCredits(showId).then(data => { setCredits(data.credits); setTypes(data.creditTypes) })
  }, [showId])
  ...
  const save = async () => {
    ...
    try {
      ...
      setCredits(data.credits); onSaved?.()
    } finally { setSaving(false) }
  }

  return <div className="credit-repeater">
    <div className="credit-repeater-heading"><div><h2>Production credits</h2><p>Keep cast and crew structured, translated and in the order shown publicly.</p></div><button className="admin-outline-button" type="button" onClick={() => setCredits(current => [...current, blankCredit(types[0])])}>Add credit</button></div>
    ...
    {credits.map((credit, index) => <article className="credit-row" key={`${credit.id ?? 'new'}-${index}`}>
      ...
      <div className="credit-fields">
        ...
        <label>Credit type<select value={credit.creditTypeId} onChange={e => typeChanged(index, e.target.value)}>{types.map(type => <option value={type.id} key={type.id}>{type.labelSq} / {type.labelEn}</option>)}</select></label>
    ...
    <div className="credit-save"><button className="admin-primary-button" type="button" disabled={saving} onClick={save}>{saving ? 'Saving…' : 'Save credits'}</button></div>
  </div>
}
