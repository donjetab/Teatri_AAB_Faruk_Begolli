...
export function CreditRepeater({ showId, value, onChange, onSaved }) {
  ...
  return <div className="credit-repeater">
    <div className="credit-repeater-heading"><div><h2>{t('Production credits')}</h2><p>{t('Keep cast and crew structured, translated and in the order shown publicly.')}</p></div><button className="admin-outline-button" type="button" disabled={!types.length} onClick={() => setCredits(current => [...current, blankCredit(types[0])])}>{t('Add credit')}</button></div>
    {!credits.length && <div className="admin-empty"><strong>{t('No credits yet')}</strong><p>{t('Add the director, playwright, cast and production crew.')}</p></div>}
    {credits.map((credit, index) => <article className="credit-row" key={`${credit.id ?? 'new'}-${index}`}>
      ...
      <div className="credit-fields">
        <label>{t('Person')}<input required value={credit.personName} onChange={e => update(index, 'personName', e.target.value)} placeholder={t('Full name')} /></label>
        <label>{t('Credit type')}<select required value={credit.creditTypeId} onChange={e => typeChanged(index, e.target.value)}>{types.map(type => <option value={type.id} key={type.id}>{type.labelSq} / {type.labelEn}</option>)}</select></label>
        <label>{t('Role in Albanian')}<input value={credit.roleSq} onChange={e => update(index, 'roleSq', e.target.value)} /></label>
        <label>{t('Role in English')}<input value={credit.roleEn} onChange={e => update(index, 'roleEn', e.target.value)} /></label>
      </div>
    ...
    {!controlled && <div className="credit-save"><button className="admin-primary-button" type="button" disabled={saving} onClick={save}>{t(saving ? 'Saving…' : 'Save credits')}</button></div>}
  </div>
}
